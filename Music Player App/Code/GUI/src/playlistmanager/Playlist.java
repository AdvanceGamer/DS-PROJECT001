...
public class Playlist implements Serializable {
    ...
    // private long totalMiliseconds = Long.MAX_VALUE;
    private long totalSeconds = Long.MAX_VALUE;
    private Timer sliderUpdateTimer = null;
    ...
    public void setRepeatMode(boolean state) {
        repeatMode = state;
    }
    ...
    public boolean getIsSongPlaybackStopped() {
        return isSongPlaybackStopped;
    }

    public boolean getIsSongPlaybackStarted() {
        return isSongPlaybackStarted;
    }
    ...
    public void setSearchAction(boolean state) {
        searchAction = state;
    }

    public void setMp3FileNull() {
        mp3File = null;
    }

    public void setPlayThreadNull() {
        playThread = null;
    }
    public void setCurrentTime(int newTime){
        currentTime=newTime;
    }
    public void setPausedFrame(int newTime){
        pausedFrame=newTime;
    }
    public Timer getSliderUpdateTimer(){
        return sliderUpdateTimer;
    }
    public void setSliderUpdateTimerNull(){
    ...
    public List<SearchResult> searchSongBySongName(String songName) {
        ...
        while (current != null) {
            if (containsIgnoreCase(current.song.getFileName(), songName)) {
                SearchResult result = new SearchResult(current.song, position);
                matchedSongs.add(result);
                // System.out.println("Found song: " + result);
                // if (wasRepeatModeOn) {
                // enableRepeatMode();
                // }
                // Return the found song and its position
            }
            current = current.next;
            position++;
        }
        ...
        return matchedSongs; // Return null if the song is not found
    }
    ...
    public void sortPlaylist(String byTitle) {
    ...
    // Merge Sort function
    private Node mergeSort(Node node, String byTitle) {
    ...
    private Node merge(Node left, Node right, String byTitle) {
        if (left == null)
            return right;
        if (right == null)
            return left;
    ...
    private int compareSongs(Song s1, Song s2, String byTitle) {
        if (byTitle == "By Name") {
            return s1.getFileName().compareToIgnoreCase(s2.getFileName());
        } else if (byTitle == "By Artist") {
    ...
    // Play Song by position in the playlist
    ...
    public void playSongByName(String songName) {
        ...
        while (current != null && current.next != null) {
            if (current.song.getFileName().equals(songName)) {
                break;
            }
            current = current.next;
        ...
        playThread = new Thread(() -> {
            ...
            while (flag) {
                ...
                if (!isPaused && currentSong != null) {
                    listener.onSongChanged(currentSong.getFileName());
    ...
    // Handle Song Finishes(played full song)

    //

    private void startSliderUpdateTimer() {
        sliderUpdateTimer = new Timer();
        // System.out.println("Timer thread started successfully" + sliderUpdateTimer);
        currentTime = pausedFrame;
        // Schedule the TimerTask to update the slider every 500 milliseconds
        sliderUpdateTimer.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
                // Update the UI on the Event Dispatch Thread (EDT)
                // System.out.println("isPaused" + isPaused + "pausedFrame" + pausedFrame);
                if (isSongPlaybackStarted && !isPaused) {
                    // Increment pausedFrame by 1 second every time this task runs
                    // System.out.println("isPaused"+isPaused+"pausedFrame"+pausedFrame);
                    currentTime = currentTime + 1000;
                    listener.simulateSeekbar(currentTime / 1000, (int) (totalSeconds));
                }
            }
        }, 0L, 1000L); // Run the task immediately and then every 500 milliseconds
    }
    ...
    public void playSong(Song song) {
        ...
        try {
            ...
            // totalFrames = mp3File.getFrameCount();
            // totalMiliseconds = mp3File.getLengthInMilliseconds();
            ...
            player.setPlayBackListener(new PlaybackListener() {
                ...
                public void playbackFinished(PlaybackEvent evt) {
                    ...
                    pausedFrame = pausedFrame + evt.getFrame(); // Save the frame for resuming
                    if (!isPaused) {

                        if (evt.getFrame() == 0) {
                            // System.out.println("evt.getFrame():" + evt.getFrame());
                            isSongPlaybackStarted = false;
                            pausedFrame = 0;
                            ...
                            if (currentSongNode.next != null) {
                                ...
                                if (sliderUpdateTimer != null) {
                                    sliderUpdateTimer.cancel();
                                    sliderUpdateTimer = null;
                                    System.out.println("Timer thread terminated successfully");
                                }
                                isSongPlaybackStopped = true;
                                mp3File = null;
                                playThread = null;
                                player = null;
                                listener.simulateSeekbar(0, 0);
                                System.out.println("Whole Playlist Played Playback Exiting....");
                            }

                        }
                    }
                    // System.out.println("Playback stopped at frame: " +
                    // (int)(pausedFrame*frameRatePerMiliseconds));
            ...
            frameRatePerMiliseconds = 1.0;
            isSongPlaybackStopped = true;
            System.out.println("Song not found Playback Stopped ");
            listener.showMessageDialog(e.getMessage() + "\n Playback Stopped");
            listener.simulateSeekbar(0, 0);
            if (sliderUpdateTimer != null) {
                sliderUpdateTimer.cancel();
                System.out.println("Timer thread terminated successfully");
    ...
    public synchronized void pauseSong() {
        if (!isPaused && isSongPlaybackStarted) {
            ...
            if (sliderUpdateTimer != null) {
                sliderUpdateTimer.cancel(); // Stop the slider update when paused
                // System.out.println("Timer Thread Terminated Successfully");
                sliderUpdateTimer = null; // Nullify so it can be re-initialized on resume
            }
            System.out.println("Playback paused.");
            // System.out.println("Playback paused at frame(milisecond): " + pausedFrame);
            // System.out.println("Playback paused at frame(second): " + pausedFrame/1000);
            // System.out.println("Playback paused at frame: " + (int) (pausedFrame * frameRatePerMiliseconds));
        }
    }
    ...
    public synchronized void resumeSong() {
        if (isPaused && isSongPlaybackStarted) {
            ...
            // System.out.println("Playback resumed from frame(milisecond): " +
            // pausedFrame);
            // System.out.println("Playback resumed from frame(milisecond): " +
            // pausedFrame/1000);
    ...
    public synchronized void nextSong() {
        if (isSongPlaybackStarted && currentSongNode != null && currentSongNode.next != null) {
    ...
    public synchronized void previousSong() {
        if (isSongPlaybackStarted && currentSongNode != null && currentSongNode.previous != null) {
            if (player != null && isPaused == false) {
                try {
                    player.stop();
                } catch (Exception e) {
    ...
    public synchronized void stopSongPlayback() {
        if (isSongPlaybackStarted) {
